...
if sys.platform == "win32":
    if (sys.version_info.major == 3 and sys.version_info.minor >= 8):
        ...
        # StreamerBot delta: guarded with isdir(), because upstream raises
        # FileNotFoundError when the sibling directory is absent, which makes the
        # module unimportable on any layout that keeps the DLL elsewhere.
        _tt_dll_dir = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "..", "TeamTalk_DLL"
        )
        if os.path.isdir(_tt_dll_dir):
